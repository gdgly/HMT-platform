...
////IO操作函数	 
//#define IIC_SCL0    PBout(6) //SCL
//#define IIC_SDA0    PBout(7) //SDA	 
//#define READ_SDA0   PBin(7)  //输入SDA 

//////驱动接口，GPIO模拟IIC
////SCL-->PB10
////SDA-->PB11
//#define SDA_IN1()  {GPIOB->CRH&=0XFFFF0FFF;GPIOB->CRH|=0x00008000;}
//#define SDA_OUT1() {GPIOB->CRH&=0XFFFF0FFF;GPIOB->CRH|=0x00003000;}
...
void IIC_Start(u8 channel);			   	//发送IIC开始信号
void IIC_Stop(u8 channel);	  			//发送IIC停止信号
void IIC_Send_Byte(u8 txd ,u8 channel);			//IIC发送一个字节
u8 IIC_Read_Byte(unsigned char ack ,u8 channel);//IIC读取一个字节
u8 IIC_Wait_Ack(u8 channel); 				//IIC等待ACK信号
void IIC_Ack(u8 channel);					//IIC发送ACK信号
void IIC_NAck(u8 channel);				//IIC不发送ACK信号
