...
//------------------------------------------------------------------
extern struct DMP_FIFO_map DMP_DATA[ChannelNum];  //数据引出				   -
...
extern float gyroxGloble[ChannelNum];
extern float gyroyGloble[ChannelNum];
//DMP API子程序
uint8_t MPU6050_DMP_Initialize(u8 Channel); //DMP初始化
void DMP_Routing(u8 Channel);	 //DMP 线程，主要用于读取和处理DMP的结果   [需要定时调用]
