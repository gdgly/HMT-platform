...
int16_t  HMC5883_FIFO[ChannelNum][3][11]; //磁力计滤波
...
void  HMC58X3_newValues(int16_t x,int16_t y,int16_t z,u8 Channel)
{
	...
	HMC5883_FIFO[Channel][0][10]=sum/10;	//将平均值更新
...
{
  char ID_temp[3];
  HMC58X3_getID(&ID_temp[0], Channel); //对ID临时数组赋值
...
{ 
  HMC5883_Check(Channel); //检测HMC5883是否存在
  HMC58X3_init(0, Channel); // Don't set mode yet, we'll do that later on.
  HMC58X3_setMode(0, Channel);
  HMC58X3_setDOR(6, Channel);  //75hz 更新率
