...
void USART1_IRQHandler(void)
{
  ...
  else if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
  {
    ...
    if(UartBuf_Cnt(&UartRxbuf)==UartRxbuf.Mask) USART_SendData(USART1, 'F');//串口接收数组长度等于掩码时，发送接收缓冲满标志
		...
		//采集数据开始结束命令判断
//		if(UartBuf_Cnt(&UartRxbuf)>=7)
//		{
//			if(UartBuf_RD(&UartRxbuf)==0xfa&&UartBuf_RD(&UartRxbuf)==0xfb)
//			{
//				switch(UartBuf_RD(&UartRxbuf))
//				{
//					case  0x01: 
//						TIM_Cmd(TIM3, ENABLE);
...
//					break;
//					case  0x04: 
//						Timpackseqnum=(UartBuf_RD(&UartRxbuf)<<24)|(UartBuf_RD(&UartRxbuf)<<16)|(UartBuf_RD(&UartRxbuf)<<8)|UartBuf_RD(&UartRxbuf);
//						Tim_Sync(Timpackseqnum);
//					break;
//				}
//			}
//		}
